...
set "sv=quake3.exe +set sv_pure 0 +set sv_maxclients 32 +set r_fullscreen 0 +set bot_enable 0"
...
rem start /D "%gamedir%" "" %sv% +devmap test_bigbox
rem start /D "%gamedir%" "" %sv% +connect localhost:27960
rem start /D "%gamedir%" "" %sv% +connect sodmod.ml:27964 +set developer 1
rem 
start /D "%gamedir%" "" %sv% +nointro
rem start /D "%gamedir%" "" %sv% +set g_gametype 0 +devmap q3dm1 +addbot anarki 5 +addbot mynx 5
rem start /D "%gamedir%" "" %sv% +set g_gametype 1 +devmap q3dm1 +addbot anarki 1
rem start /D "%gamedir%" "" %sv% +set g_gametype 3 +devmap q3dm6 %rb% %rb% %bb% %bb% %fs% +set bot_minplayers 10
